...
function parseSize(rawDateAndSize) {
  ...
  if (match !== null && match.length > 1)  {
    ...
    var sizeInMb = Math.round(number);
    return sizeInMb;
  } else {
...
tpb.extractTorrentDataForSinglePage = function (html) {
  ...
  $('.vertTh', tableResult).each(function (index, element) {
    ...
    if (!_.startsWith(magnet, 'magnet:')) {
      //  url, starts again
      // crawlerUtils.attemptDataExtractionFromUrl("https://ukpirate.org" +magnet, tpb.extractTorrentDataForSinglePage, currentData).then(function(data) {
      //   console.log("Closure inside THEN - ", data);
      // }).catch(function(err) {
      //   console.log("Error");
      // });
    ...
    try {
      ...
      console.log("Error parsing size " + size);
    }
