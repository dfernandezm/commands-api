...
if [[ "$TVSTER_MODE" == "organizer" ]]; then
    ...
    IMAGE=dfernandez/tvster:tvster-organizer-v1.0.1.2
...
#    mkdir -p /tmp/mediacentertest/temp
    ...
    VOLUMES="$VOLUMES -v /mediacenter:/mediacenter"
    echo "$VOLUMES"
    USERNAME=
    PASSWORD=
else
    ...
    IMAGE=dfernandez/tvster:tvster-api-local
